...
# ---------------------------------------------------------
# 9. Model Training using Regularized Logistic Regression Variants
#'ridge' (L2), 'lasso' (L1), or 'elasticnet' (combination)
reg_variant = 'ridge'

if reg_variant == 'ridge':
    model = LogisticRegression(penalty='l2', random_state=42, max_iter=1000)
elif reg_variant == 'lasso':
    # For L1 penalty, use solver 'liblinear' or 'saga'
    model = LogisticRegression(penalty='l1', solver='saga', random_state=42, max_iter=1000)
elif reg_variant == 'elasticnet':
    # For elasticnet, use solver 'saga' and set l1_ratio (between 0 and 1)
    model = LogisticRegression(penalty='elasticnet', solver='saga', l1_ratio=0.5, random_state=42, max_iter=1000)
else:
    raise ValueError("Invalid reg_variant specified. Choose from 'ridge', 'lasso', or 'elasticnet'.")
...
# ---------------------------------------------------------
# 10. Explainability with SHAP using LinearExplainer
...
iface_explanations = gr.Interface(
    ...
    title="Alzheimer's Prediction using Explainable AI techniques - SHAP, PDP, and ICE (LR With L2 Regularization)",
