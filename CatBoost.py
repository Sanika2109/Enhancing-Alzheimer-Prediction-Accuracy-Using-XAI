...
# Then, split the training+validation into 75% training and 25% validation.
# Result: ~60% training, ~20% validation, ~20% testing.
X_train, X_val, y_train, y_val = train_test_split(X_temp, y_temp, test_size=0.25, random_state=42)

# ---------------------------------------------------------
# 9. Model Training using CatBoost and Cross Validation
# Initialize CatBoostClassifier with verbose output disabled.
...
# CatBoost is tree-based so we can use TreeExplainer.
...
def generate_explanations(age, gender, mmse, etiv, nwbv, delta_mmse):
    ...
    # For tree-based classifiers, TreeExplainer returns a list (one per class); use the positive class.
    ...
    plt.barh(range(len(sorted_idx)), shap_val[sorted_idx], align='center')
    plt.yticks(range(len(sorted_idx)), np.array(selected_features)[sorted_idx])
    plt.xlabel("SHAP Value")
    plt.title("SHAP Bar Plot for Given Input")
    plt.savefig("shap_bar_plot.png", bbox_inches="tight")
    plt.close()

    return "shap_plot_dynamic.png", "pdp_ice_explanation.png", "shap_bar_plot.png"

# ---------------------------------------------------------
# 12. Create the Gradio Interface using gr.Interface
iface = gr.Interface(
    ...
    outputs=[
        ...
        gr.Image(label="SHAP Bar Plot")
    ],
    title="Alzheimer's Explainable AI with SHAP, PDP, ICE, and Bar Plot (CB Classifier with CV)",
    description="Enter patient details to generate a SHAP force plot (local explanation), PDP with ICE curves (global explanation), and a SHAP bar plot for key features."
)
